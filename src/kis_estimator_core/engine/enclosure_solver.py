...
class EnclosureSolver:
    """Enclosure selection and optimization solver"""

    def __init__(self, work_dir: Optional[Path] = None, rules_dir: Optional[Path] = None):
        self.work_dir = work_dir or Path("./work")
        self.rules_dir = rules_dir or Path("./rules")

    def solve(self, spec: Dict[str, Any]) -> Dict[str, Any]:
        """Solve for optimal enclosure configuration"""
        return calculate_enclosure(self.work_dir, self.rules_dir)
