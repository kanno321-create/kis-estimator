...
class EstimateFormatter:
    """Estimate document formatter for Excel and PDF generation"""

    def __init__(self, work_dir: Optional[Path] = None):
        self.work_dir = work_dir or Path("./work")

    def format(self, data: Dict[str, Any], format_type: str = "excel") -> Dict[str, Any]:
        """Format estimate data for output"""
        if format_type == "excel":
            return format_estimate_excel(self.work_dir)
        elif format_type == "pdf":
            return format_estimate_pdf(self.work_dir)
        else:
